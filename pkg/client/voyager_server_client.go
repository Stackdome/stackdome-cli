...
func NewClient(token string, serverURL string) VoyagerServerClient {
	return &vclient{AccessToken: token}
}
...
func (c *vclient) GetUserInfo(ctx context.Context) (*authentication.AuthResponse, error) {
	...
	return &authentication.AuthResponse{
		Username:  "ashish",
		Team:      "voyager-labs-dev",
		Namespace: "ashish-workspace",
		// 1 year
		TokenValidTill: time.Now().Add(time.Hour * 365),
	}, nil
}
