...
func TestResolveDefaultProjectFallsBackWhenCurrentUserProjectsHaveNoName(t *testing.T) {
	var paths []string
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/api/v1/users/current/projects":
			fmt.Fprint(w, `{"items":[{"project":{"name":"default","default_project":true}}],"total":1}`)
		case "/api/v1/organizations/org-1/projects":
			fmt.Fprint(w, `{"items":[{"name":"other"},{"name":"the-default","default_project":true}],"total":2}`)
		default:
			t.Errorf("unexpected request to %s", r.URL.Path)
...
func TestResolveDefaultProjectErrorsWhenNoProjectHasAName(t *testing.T) {
	var paths []string
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/api/v1/users/current/projects":
			fmt.Fprint(w, `{"items":[{"project":{"id":"project-1"}}],"total":1}`)
		case "/api/v1/organizations/org-1/projects":
			fmt.Fprint(w, `{"items":[{"id":"project-1"}],"total":1}`)
		default:
			t.Errorf("unexpected request to %s", r.URL.Path)
			w.WriteHeader(http.StatusNotFound)
		}
	...
	name, err := c.ResolveDefaultProject(context.Background(), "org-1")
	if err == nil {
		t.Fatal("ResolveDefaultProject returned a nil error for nameless projects")
	}
	if name != "" {
		t.Errorf("project = %q, want empty on error", name)
	}
	if len(paths) != 2 || paths[1] != "/api/v1/organizations/org-1/projects" {
		t.Errorf("requests = %v, want the org projects endpoint consulted second", paths)
	}
